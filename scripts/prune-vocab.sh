#!/bin/bash
# Wrapper for prune_vocab.py: prunes unused BPE vocab entries from a GGUF
# based on real usage (tokenized with the model's own tokenizer) plus the
# BPE merge-chain closure, running inside the turboquant docker image since
# it needs both gguf-py and the compiled llama-tokenize binary.
#
# Usage: prune-vocab.sh <input.gguf> <output.gguf> --corpus <file1> [file2 ...] [--force]
#
...
INPUT="${1:?Usage: prune-vocab.sh <input.gguf> <output.gguf> --corpus <file1> [file2 ...] [--force]}"
...
docker run --rm -i \
    ...
    "$IMAGE" \
    /host-scripts/prune_vocab.py "$INPUT_ARG" "$OUTPUT_ARG" --corpus "${CORPUS_ARGS[@]}" "${EXTRA_ARGS[@]}"
